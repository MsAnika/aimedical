...
if r.status_code == 200:
    ...
    if r2.status_code == 200:
        ...
        download = requests.get(download_url, headers={'Authorization': f'Bearer {token}'})
        print(f'Download Status: {download.status_code}')
        print(f'Download size: {len(download.content)} bytes')
        print(f'Download filename: {download.headers.get("Content-Disposition", "unknown")}')